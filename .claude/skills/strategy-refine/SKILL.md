...
**Detecting unrefined stubs**: A strategy is unrefined if its `## Strategy (AI Generated by Agentic SDLC Pipeline)` section is empty or missing. Do NOT use the presence of a `## Strategy` header alone — cloned STRATs from Jira may have a bare `## Strategy` header with no content. Check that the section has actual substantive content (implementation approach, components, effort estimates). If it doesn't, treat it as an unrefined stub that needs initial refinement.
