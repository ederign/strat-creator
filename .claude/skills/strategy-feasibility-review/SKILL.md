...
**Multi-architecture support**: RHOAI ships on four CPU architectures: amd64, arm64, ppc64le, and s390x. When a strategy introduces new container images with compiled native dependencies (e.g., PyTorch, OpenBLAS, LLVM), building for ppc64le and s390x often requires architecture-specific build scripts, source compilation of dependencies, and custom Dockerfile stages. This build complexity is significant effort that is frequently absent from estimates. If a component has known partial architecture support (fewer than four), the strategy should state which architectures are targeted.
